...
if has('nvim')
  ...
  function! lf#EditCallback(job_id, code, event)
...
function! lf#LF(path, edit_cmd, user_args)
  ...
  if has('nvim')
    let termopen_opts = { }
    let termopen_opts.on_exit = function('lf#EditCallback')
    enew
    ...
    exe 'autocmd! BufWinLeave <buffer='.bufn.'> call lf#EditCallback()'
  endif
